...
- `skills/system-design/references/*.md` — seventeen self-contained topic files. The agent loads only the one a task needs.
...
- `README.md` — human-facing install + overview. `LICENSE` — MIT. `ROADMAP.md` — the paced improvement queue (one increment per day or two; update its status column when an increment ships).
...
`low-level-design` · `case-studies` · `networking` · `os-concurrency` · `payments` ·
`ai-ml-systems` · `dev-tools` · `interview`
