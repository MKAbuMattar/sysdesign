...
2. **Capacity estimate** — QPS (average + peak), storage/day and /year, bandwidth, cache working set. One line of arithmetic each; round hard. Name the bottleneck resource that drives the design.
...
5. **High-level architecture** — a Mermaid diagram (clients → edge → services → data + cache + queue) plus the critical path and the async paths.
